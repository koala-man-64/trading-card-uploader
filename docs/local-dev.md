...
`SCANNER_ADMIN_BASE_URL` must point to the scanner host root for processed and segmented gallery operations, for example `http://127.0.0.1:7072` for a local scanner Function host. `SCANNER_TIMEOUT_SECONDS` defaults to `30`; keep it long enough for local or cold-started scanner gallery requests.
