...
.\scripts\Initialize-DevPhoneEnvironment.ps1 `
  ...
  -ScannerAdminBaseUrl https://<scanner-function>.azurewebsites.net `
  -GrantAdminConsent `
  -AssignAzureRoles
```
...
2. Run `infra-ci` with `deployDev=false` and review the dev what-if. The workflow fails fast if `SCANNER_ADMIN_BASE_URL` is blank.
