...
For deep reviews or refactors, read `.codex/skills/fail-fast-enforcement-agent/references/strict-failure-checklist.md`.
...
## Findings Taxonomy
Validation, Type Safety, Contract Semantics, Exception Handling, Logging, Resource Cleanup, Retry, Timeout, State Integrity, Async, Naming Honesty, Test Coverage, Consistency.

## Severity model
- Critical: false success, hidden data loss, corrupted state, invalid startup/readiness, masked dependency failure, security-relevant failure masking
- High: silent fallback, swallowed exception, weak type boundary causing likely misuse, ambiguous contract, orchestration continuing after required failure
- Medium: imprecise diagnostics, inconsistent failure typing, missing cleanup, retry semantics that obscure root cause
