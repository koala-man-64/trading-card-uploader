---
name: gateway-agent
description: "Coordinate autonomous workers through an append-only project ledger at PROJECT_ROOT/gateway_agent_ledger.csv. Use when acting as a gateway or dispatcher that must claim folders, record heartbeats or progress, mark blockers, hand off work, detect conflicting ownership, or close work items without letting worker agents write directly to the ledger."
---
...
- Read `.codex/skills/gateway-agent/references/ledger-spec.md` when you need the full schema, field semantics, or state-derivation rules.
- Use `.codex/skills/gateway-agent/scripts/gateway_ledger.py` instead of hand-editing CSV rows.
